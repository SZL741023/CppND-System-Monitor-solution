...
string LinuxParser::Ram(int pid) {
  ...
  if (stream.is_open()) {
    while (std::getline(stream, line)) {
      ...
      if (item == "VmSize") {
        linestream >> memStr;
        return std::to_string(stoi(memStr) / 1024);
      }
    }
  }
}
